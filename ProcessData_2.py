...
def applyWindow(df, col_name, window_size):
    '''
        Apply moving window average on the data (low-pass filter)
    '''
    temp_df = df[col_name].copy()
    temp_df = temp_df.rolling(window_size, center=True, min_periods=1).mean()
...
def processPupil(main_path, info_path, video_path, output_path):
    '''
        Process data for eye tracking
    '''
    # Import timestamps
    ...
    # Denoising
    CRP_smooth = applyWindow(CRP_clean,['xc','yc','a','b','x','y','eye_center_x','eye_center_y','pupil_x_proj','pupil_y_proj'],5)
    ...
    output_file = os.path.join(output_path, 'CRP.csv')
    CRP_velocity.to_csv(output_file,index=False)
    
def processMPU(info_path, output_path,calibration_path):
    '''
        Process data for head tracking
    '''
    # Earth fravity contant
    g = 9.80665

    # Import timestamp
    timestamp = pd.read_csv(os.path.join(info_path,'accel.csv')).drop(columns=['ax','ay','az'])

    # Import accelerometer
    accel = pd.read_csv(os.path.join(info_path,'accel.csv')).drop(columns=['Time'])
    cal_accel = pd.read_csv(os.path.join(calibration_path,'cal_accel.csv'))
    cal_accel = [cal_accel.iloc[0]['x'],cal_accel.iloc[0]['y'],cal_accel.iloc[0]['z']]
    accel = accel.sub(cal_accel,axis='columns').div(g)
    # accel = accel.div(g)

    # Import gyroscope
    gyro = pd.read_csv(os.path.join(info_path,'gyro.csv')).drop(columns=['Time'])
    cal_gyro = pd.read_csv(os.path.join(calibration_path,'cal_gyro.csv'))
    cal_gyro = [cal_gyro.iloc[0]['x'],cal_gyro.iloc[0]['y'],cal_gyro.iloc[0]['z']]
    gyro = gyro.sub(cal_gyro,axis='columns')
    
    # Create dataframe
    mpu = pd.concat([timestamp,accel,gyro],axis=1).dropna()
    
    # Apply denoising (first low pass)
    mpu_window = applyWindow(mpu, ['ax','ay','az','gx','gy','gz'],40)
    
    # Get (angular) velocity
    # mpu_velocity = mpuVelocity(mpu_window,50)

    # Save processed Data
    if not os.path.exists(output_path):
        os.makedirs(output_path)
        
    output_file = os.path.join(output_path, 'mpu.csv')
    mpu_window.to_csv(output_file,index=False)

def processHalf(main_path, info_path, video_path, output_path,calibration_path):
    '''
        Processing pipeling for low and high protocol
    '''
    # Process pupil data
    processPupil(main_path, info_path, video_path, output_path)
    
    # Process accelerometer data
    processMPU(info_path, output_path,calibration_path)
    
# Main function
def main(main_path, info_path, video_path, output_path,calibration_path):
    # Process low protocol
    processHalf(main_path['low'], info_path['low'], video_path['low'], output_path['low'],calibration_path)
    
    # Process high protocol
    processHalf(main_path['high'], info_path['high'], video_path['high'], output_path['high'],calibration_path)
...
if __name__ == '__main__':
    ...
    calibration_path = r'C:\Users\HeLab\Documents\Ruijia\Project\EyeTracking\Data\v2\_Surgery_Calibration'
    
    ### Call main() function
    main(main_path, info_path, video_path, output_path,calibration_path)